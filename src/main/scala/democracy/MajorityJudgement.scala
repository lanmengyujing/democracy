...
object Grade:
  ...
  def median(grades: Seq[Grade]): Grade =
     ...
     val gradeLength = grades.size
     sortedGrades(gradeLength/2)
end Grade
  ...
  def findWinner(gradesPerCandidate: Map[Candidate, Seq[Grade]]): Candidate =
    ...
    if gradesPerCandidate.forall((candidate, grades) => grades.isEmpty) then
      ...
      val bestMedianGrade: Grade =
        val allGrades = gradesPerCandidate.values;
        val allMediaGrade = allGrades.filter(_.nonEmpty).map((grades => Grade.median(grades)))
        allMediaGrade.maxBy(_.ordinal)
